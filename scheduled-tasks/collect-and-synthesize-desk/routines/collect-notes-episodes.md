...
description: Collect Apple Notes.app notes I edited since the previous successful run into Obsidian _episodes/
model: sonnet
effort: medium
---

Collect Apple Notes.app activity since the previous successful run and write one episode file per modified note into the Obsidian vault.
...
- STATE_FILE: /Users/joey/.claude/scheduled-tasks/collect-and-synthesize-desk/state/collect-notes-episodes.txt
...
- Compute `lookback_seconds` = (fire_time − window_start) in whole seconds, and pass that into the AppleScript snippet below.
- At END, ONLY after the run has completed without error, overwrite STATE_FILE with `fire_time` formatted as ISO 8601 with offset. If AppleScript was blocked or the run errored partway, DO NOT update STATE_FILE so the next run picks back up from the same point.
...
Use this AppleScript via Bash (osascript -e ...) to enumerate notes modified since `window_start`. Substitute `<LOOKBACK_SECONDS>` with the value computed above:

  tell application "Notes"
    set cutoff to (current date) - (<LOOKBACK_SECONDS>)
    set results to {}
...
- If AppleScript is blocked by macOS Automation permissions, write a single file YYYY-MM-DD-notes-NORUN.md with body "Notes.app access denied — grant Automation permission to the terminal/Claude Code in System Settings > Privacy & Security > Automation" and exit cleanly. Do NOT update STATE_FILE in this case.
...
Report: count written, count skipped (unchanged), count overwritten (re-edited), any AppleScript errors, the `window_start`/`window_end` used, and whether STATE_FILE was updated.
