...
2. **Review the plan before building** 🔒 — **`solve-in-worktrees`** Phase 2b: Codex, Gemini (`gemini-3.1-pro-high`, its most capable slug), and Cursor (`claude-opus-5-high`) review the same plan concurrently against the real codebase — requirement coverage, root cause vs symptom, reuse it's reinventing, blast radius, ambiguity. Cap 2 rounds. It also checks the plan is concrete enough to write tests against, since the approved plan — not the built code — is what the test agents work from. Ambiguity findings come back to you; the approved plan goes to the build agent **and** to T1 at the same moment.
